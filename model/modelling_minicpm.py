...
class MiniCPMRotaryEmbedding(nn.Cell):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, precision=mindspore.float16):
        super().__init__()
        ...
        self._set_cos_sin_cache(
            seq_len=max_position_embeddings, dtype=precision
        )
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=2):
    ...
    """
    # q, k : (seq_len, bsz, num_heads, head_dim)
    # position_ids: (seq_len, bsz)
    # cos, sin : (seq_len, head_dim) --> (seq_len, bsz, head_dim) --> (seq_len, bsz, 1, head_dim)
    # q_embed, k_embed: (seq_len, bsz, num_heads, head_dim)
    orig_dtype = k.dtype
    cos = cos[position_ids].unsqueeze(unsqueeze_dim)  # (seq_len, bsz, 1, head_dim)
    sin = sin[position_ids].unsqueeze(unsqueeze_dim)  # (seq_len, bsz, 1, head_dim)
...
def repeat_kv(hidden_states: mindspore.Tensor, n_rep: int) -> mindspore.Tensor:
    """
    This is the equivalent of ops.repeat_interleave(input, repeats=n_rep, axis=2). The hidden states go from (seq_len,
    bsz, num_key_value_heads, head_dim) to (seq_len, bsz, num_heads, head_dim)
    """
    slen, batch, num_key_value_heads, head_dim = hidden_states.shape
    if n_rep == 1:
        return hidden_states
    hidden_states = hidden_states[:, :, :, None, :].broadcast_to((slen, batch, num_key_value_heads, n_rep, head_dim))
...
class MiniCPMAttention(nn.Cell):
    ...
    def __init__(self, config: MiniCPMConfig, 
        ...
        # cache_k: (seq_len, bsz, num_heads, head_dim)
        self.cache_k = mindspore.Parameter(initializer('zeros',
                                (self.max_position_embeddings, max_batch_size, self.num_heads, self.head_dim),
                                params_dtype), 'cache_k', requires_grad=False)

        self.cache_v = mindspore.Parameter(initializer('zeros',
                                (self.max_position_embeddings, max_batch_size, self.num_heads, self.head_dim),
    ...
    def construct(
        ...
        **kwargs,
        ...
        """
        hidden_states: (q_len, bsz, hidden_size)
        attention_mask: ()
        ...
        if self.config.pretraining_tp > 1:
            ...
            # query_states: (q_len, bsz, num_heads * head_dim)
            # key_states, value_states: (q_len, bsz, num_key_value_heads * head_dim)
            query_states = self.q_proj(hidden_states)
            key_states = self.k_proj(hidden_states)
            value_states = self.v_proj(hidden_states)

        # query_states: (q_len, bsz, num_heads * head_dim) --> (q_len, bsz, num_heads, head_dim)
        # key_states, query_states: (q_len, bsz, num_key_value_heads * head_dim) --> (q_len, bsz, num_key_value_heads, head_dim)
        query_states = query_states.view(q_len, bsz, self.num_heads, self.head_dim)
        key_states = key_states.view(q_len, bsz, self.num_key_value_heads, self.head_dim)
        value_states = value_states.view(q_len, bsz, self.num_key_value_heads, self.head_dim)
        
        # cos, sin : (q_len, bsz, 1, head_dim)
        cos, sin = self.rotary_emb(value_states.to(mindspore.float32), seq_len=kv_seq_len)

        # position_ids: (bsz, q_len) --> (q_len, bsz)
        position_ids = position_ids.swapaxes(0, 1)

        # query_states: (q_len, bsz, num_heads, head_dim)
        # key_states: (k_v_len, bsz, num_key_value_heads, head_dim)
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids)

        # key_states, value_states: (k_v_len, bsz, num_key_value_heads, head_dim) --> (k_v_len, bsz, num_heads, head_dim)
        key_states = repeat_kv(key_states, self.num_key_value_groups)
        value_states = repeat_kv(value_states, self.num_key_value_groups)

        kv_seq_len = key_states.shape[0]
        if kv_seq_len > 1:
            indices = ops.arange(start_pos, kv_seq_len, dtype=mindspore.int64)
        else:
            indices = start_pos.expand_dims(0)

        ops.scatter_update(self.cache_k, indices, key_states)
        ops.scatter_update(self.cache_v, indices, value_states)

        # key_states, value_states: (k_v_len, bsz, num_heads, head_dim)
        key_states = self.cache_k
        value_states = self.cache_v

        # (bsz, num_heads, q_len, k_v_len)
        attn_weights_output_size = (query_states.shape[1], query_states.shape[2], query_states.shape[0], key_states.shape[0])

        # query_states: (q_len, bsz, num_heads, head_dim) --> (q_len, bsz * num_heads, head_dim)
        query_states = query_states.view((attn_weights_output_size[2], attn_weights_output_size[0] * attn_weights_output_size[1], -1))
        # key_states: (k_v_len, bsz, num_heads, head_dim) --> (k_v_len, bsz * num_heads, head_dim)
        key_states = key_states.view((attn_weights_output_size[3], attn_weights_output_size[0] * attn_weights_output_size[1], -1))

        # query_states: (q_len, bsz * num_heads, head_dim) --> (bsz * num_heads, q_len, head_dim)
        # key_states: (k_v_len, bsz * num_heads, head_dim) --> (bsz * num_heads, head_dim, k_v_len)
        attn_weights = ops.matmul(query_states.swapaxes(0, 1), key_states.transpose(1, 2, 0)) / math.sqrt(self.head_dim)
        attn_weights = attn_weights.view(attn_weights_output_size)
        if attn_weights.shape != attn_weights_output_size:
            raise ValueError(
                f"Attention weights should be of size {attn_weights_output_size}, but is"
        ...
        attn_output_size = (value_states.shape[1], value_states[2], query_states.shape[0], value_states[3])

        # value_states: (k_v_len, bsz, num_heads, head_dim) --> (k_v_len, bsz * num_heads, head_dim)
        # attn_weights: (bsz, num_heads, q_len, k_v_len) --> (bsz * num_heads, q_len, k_v_len)
        value_states = value_states.view((attn_output_size[2], attn_output_size[0] * attn_output_size[1], -1))
        attn_weights = attn_weights.view((attn_weights_output_size[0] * attn_weights_output_size[1], attn_weights_output_size[2], -1))

        attn_output = ops.matmul(attn_weights, value_states.swapaxes(0, 1))
        attn_output = attn_output.view(attn_output_size)

        if attn_output.shape != attn_output_size:
            raise ValueError(
                f"`attn_output` should be of size {attn_output_size}, but is"
                f" {attn_output.shape}"
            )

        # attn_output: (bsz, num_heads, q_len, head_dim) --> (q_len, bsz, num_heads, head_dim)
        attn_output = attn_output.transpose(2, 0, 1, 3)
        # attn_output: (q_len, bsz, num_heads, head_dim) --> (q_len, bsz, hidden_size)
        ...
        **kwargs,
        ...
        Args:
            ...
            use_cache (`bool`, *optional*):
                ...
                (see `past_key_values`).
            start_pos (`int`, *optional*): Starting position of KV cache.
        """
        if "padding_mask" in kwargs:
            warnings.warn(
                "Passing `padding_mask` is deprecated and will be removed in v4.37. Please make sure use `attention_mask` instead.`"
...
class MiniCPMPreTrainedModel(nn.Cell):
    ...
    def _init_weights(self, cell):
        if isinstance(cell, nn.Embedding):
                cell.weight.set_data(initializer(Normal(sigma=self.std, mean=0.0), cell.weight.shape, cell.weight.dtype))
        elif isinstance(cell, nn.Dense): 
            cell.weight.set_data(initializer(Normal(sigma=self.std, mean=0.0), cell.weight.shape, cell.weight.dytpe))
